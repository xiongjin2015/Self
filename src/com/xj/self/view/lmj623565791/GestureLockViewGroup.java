...
import com.xj.self.view.lmj623565791.GestureLockView.Mode;

/**
 * 自定义手势锁View
 * 整体思路：
 * 1.自定义了一个RelatvieLayout(GestureLockViewGroup)，在里面会根据传入的每行的个数，生成多个GestureLockView(就是上面一个个小圈圈）；
 *   然后会自动进行布局，里面的宽度，间距，内圆的直径，箭头的大小神马的都是百分比实现的，所以大胆的设置你喜欢的个数，只要你没有密集恐惧症~
 * 2.GestureLockView有三个状态:没有手指触碰、手指触碰、和手指抬起，会根据这三个状态绘制不同的效果，以及抬起时的小箭头需要旋转的角度，
 *   会更加用户选择的GestureLockView，进行计算，在GestureLockViewGroup为每个GestureLockView设置
 * 3.GestureLockViewGroup主要就是判断用户ACTION_MOVE,ACTION_DOWN,ACTION_UP时改变选中的GestureLockView的状态，并且记录下来，提供一定的回调。
 * 
 * 整体包含n*n个GestureLockView,每个GestureLockView间间隔mMarginBetweenLockView,最外层的GestureLockView与容器存在mMarginBetweenLockView的外边距
 * 
 * 关于GestureLockView的边长（n*n）:n*mGestureLockViewWidth + (n+1)*mMarginBetweenLockView = mWidth; 得：mGestureLockViewWidth = 
 *  4*mWidth / (5*mCount+1),注：mMarginBetweenLockView = mGestureLockViewWidth * 0.25;
 *
 */
public class GestureLockViewGroup extends RelativeLayout {
    
    private final static String TAG = "GestureLockViewGroup";
    ...
    public GestureLockViewGroup(Context context, AttributeSet attrs, int defStyle) {
        ...
        /**获得所有自定义的参数值*/
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.GestureLockVieGroup, defStyle, 0);
        int n = a.getIndexCount();
        
        for(int i=0;i<n;i++){
            int attr = a.getIndex(i);
            switch (attr) {
                case R.styleable.GestureLockVieGroup_color_no_finger_inner_circle:
                    mNoFingerInnerCircleColor = a.getColor(attr, mNoFingerInnerCircleColor);
                    break;
                case R.styleable.GestureLockVieGroup_color_no_finger_outer_circle:
                    mNoFingerOuterCircleColor = a.getColor(attr, mNoFingerOuterCircleColor);
                    break;
                case R.styleable.GestureLockVieGroup_color_finger_on:
                    mFingerOnColor = a.getColor(attr, mFingerOnColor);
                    break;
                case R.styleable.GestureLockVieGroup_color_finger_up:
                    mFingerUpColor = a.getColor(attr, mFingerUpColor);
                    break;
                case R.styleable.GestureLockVieGroup_count:
                    mCount = a.getInt(attr, 3);
                    break;
                case R.styleable.GestureLockVieGroup_tryTimes:
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(mGestureLockViews == null){
            ...
            for(int i=0;i<mGestureLockViews.length;i++){
                ...
                /**每个View都有右外边距和底外边距，每一行的有上外边距，第一列的有左外边距*/
                if(i>=0 && i<mCount){//第一行
